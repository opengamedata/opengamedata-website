...
if (isset($_GET['game']) && $_GET['game'] != '') {
    ...
    if (isset($response_obj) && $response_obj->{'success'}) {
        ...
        $button_raw = new PipelineButton('Raw Data', 'pipeline-raw-primary.svg', 'btn-pipeline-1', 'raw', $game_files->getRawFile(), $month_name, 'Time-sequenced data as provided by the game directly. Includes player events, system feedback and game progression.');
        $button_event = new PipelineButton('Calculated Events', 'pipeline-event-primary.svg', 'btn-pipeline-2', 'event', $game_files->getEventsFile(), $month_name, 'Raw time-sequenced data interwoven with with events generated by automated detectors.');

...
export default function timeline(svg, data, eventOnClick, timelineZoom, timelinePan, setZoom, setPan) {
    ...
    var sequence = svg.append('g').classed('event-wrapper', true).attr('transform', 'translate(' + timelinePan + ')');

    // console.log(Object.entries(data.timestamps))

    var timestamp = sequence.selectAll('g').data(data.timestamps).join('g').classed('timestamp', true).attr('transform', function (d, i) {
        var tstamp = d[0];
        console.log(d);
    ...
    timestamp.selectAll('circle').data(function (d) {
        return d[1];
    }).join('circle').attr('transform', function (d, i) {
        return 'translate(0,' + i * dotSize + ')';
    });
    ...
    function handleZoom(e) {
        ...
        // pan behavior
        d3.select('.event-wrapper').attr('transform', 'translate(' + panLv + ')');
        ...
        .attr('transform', function (_ref6) {
            ...
            return 'translate(' + sacleFactorX * timestamp * zoomLv + ',0)';
        });
